...
//        }
        ...
        // Agg = Integer.parseInt(ContactsContract.Contacts.Data.getage(user));
        // Gen = Integer.parseInt(Data.getgender(user));

        // Gen: Male = 1, Female = 2
        SharedPreferencesUtils sp = new SharedPreferencesUtils(this);
        Hei = (Integer) sp.getParam("height", 180);
        Wei = (Integer) sp.getParam("weight", 90);
        if (((String) sp.getParam("gender", "Male")).equals("Male")) {
            Gen = 1;
        } else {
            Gen = 2;
        }
        ...
        born.setTime((Date)sp.getParam("birth_date", new Long(0)));
        Agg = now.get(Calendar.YEAR) - born.get(Calendar.YEAR);
        if (born.get(Calendar.DAY_OF_YEAR) > now.get(Calendar.DAY_OF_YEAR)) {
            Agg--;
        }
        if(Gen == 1){
            Q=5;
...
//        super.onBackPressed();
//        Intent i = new Intent(BloodPressureProcess.this, HealthFunctions.class);
//        startActivity(i);
//        finish();
//    }
}