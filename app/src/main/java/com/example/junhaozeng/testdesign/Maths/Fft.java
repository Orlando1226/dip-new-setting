package com.example.junhaozeng.testdesign.Maths;
...
public class Fft {
	...
	public static double FFT(Double[] in, int size, double samplingFrequency) {
		...
		for(int p=60; p<size; p++) {// 12 was chosen because it is a minimum frequency that we think people can get to determine heart rate.
        	if(temp < output[p]) {
        		temp = output[p];
        		POMP = p;
