...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initTodayData() {
        CURDATE = getDate();
        int steps = dbManager.readStepRecord(CURDATE);
        if (steps == -1) {
            CURSTEPS = 0;
        } else if (steps >= 0) {
            CURSTEPS = steps;
    ...
    private void save() {
        ...
        int dbSteps = dbManager.readStepRecord(CURDATE);
        if (dbSteps == -1) {
            dbManager.insertStepRecord(CURDATE, tempSteps);
        } else if (dbSteps >= 0) {
            // TODO:
            // create a set method in DbManager
            // set the date and step
            dbManager.updateStepRecord(CURDATE, tempSteps);
        }
    }
